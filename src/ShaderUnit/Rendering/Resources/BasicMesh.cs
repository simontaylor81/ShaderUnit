...
namespace ShaderUnit.Rendering.Resources
{
	...
	class BasicMesh
	{
		...
		public static RenderMesh CreateCube(Device device)
		{
			// Generate vertices.
			int numVerts = 24;
			...
			return new RenderMesh(device, vertices, SceneVertex.GetStride(), indices, InputElements);
		}

		// Create single square face in the XZ plane with corners at 1 (edge length 2).
		public static RenderMesh CreatePlane(Device device)
		{
			// Generate vertices.
			int numVerts = 4;
			int vertexBufferSize = SceneVertex.GetStride() * numVerts;
		...
		public static RenderMesh CreateSphere(Device device, int slices, int stacks)
		{
			int i;
